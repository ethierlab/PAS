...
% 
% USAGE: 
%   EMG_plot ( panel, aggregatedData, emgVect, xLimit, xRange, yRange, ... 
%              unrectFlag, acuteFlag, acutePreIndex, acutePostIndex, save )
% 
% USAGE EXAMPLES: 
...
function [ ] = EMG_plot ( panel, aggregatedData, emgVect, xLimit, xRange,...
    yRange, unrectFlag, acuteFlag, acutePreIndex, acutePostIndex, save ) 

    num_chan = length(emgVect);
    ...
    for ch_idx=1:num_chan
        ...
        if acuteFlag == 1
            ...
            % This break is here as the intent of the acute subsection is
            % only to plot the intended two sessions, so no iteration over
        ...
        for sess = 1:num_sess
            ...
            if unrectFlag == 1
                ...
                if save == 1
                    saveas(gcf, [aggregatedData(1).blockname '_ch' ...
                        num2str(ch) '_sess' num2str(sess) '_UNRECT_EMG.svg']);
                    savefig(gcf, [aggregatedData(1).blockname '_ch' ...
                        num2str(ch) '_sess' num2str(sess) '_UNRECT_EMG.fig']);
                end
            ...
            elseif panel == 1
                ...
                plot(aggregatedData(sess).time_axis, ...
                    aggregatedData(sess).mean_collapsed_EMGs(:,ch));
                xlabel('Time (s)'); ylabel('Mean Rectified EMG Signal (V)');
                ylim(yRange);
                legend(aggregatedData(sess).blockname);
                title(strrep(sprintf('Mean Rect EMG Ch %d',ch),'_','\_'));
                ...
                if save == 1
                    saveas(gcf, [aggregatedData(sess).blockname ...
                        '_ch' num2str(ch) '_sess' num2str(sess) '_EMG.svg']);
                    savefig(gcf, [aggregatedData(sess).blockname ...
                        '_ch' num2str(ch) '_sess' num2str(sess) '_EMG.fig']);
                end
